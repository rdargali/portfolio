(function($) {
  ...
  NAY.Accordion = function() {
    $(".accordion").each(function(i, elem) {
      ...
      $acsnav
        .first()
        .parent()
        .addClass("acco-active");
      $acsnav.on("click", function() {
        if (
          !$(this)
            .parent()
            .hasClass("acco-active")
        ) {
  ...
  NAY.scrollBar = function() {
    if ($(".scroll-bar").exists()) {
      loadScript(
        plugin_track + "scroll/jquery.mCustomScrollbar.min.js",
        function() {
...
function copyText() {
  let clipboard = document.getElementById("email");
  let emailHover = document.getElementById("emailHover");
  clipboard.select();
  document.execCommand("copy");
  emailHover.title = "Copied to clipboard!";
}
